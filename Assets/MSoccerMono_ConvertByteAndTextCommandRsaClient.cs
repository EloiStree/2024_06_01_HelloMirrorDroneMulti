...
public class MSoccerMono_ConvertByteAndTextCommandRsaClient : NetworkBehaviour {
    ...
    public void PushActionAsByteOnClient(byte[] byteCommand) {
        ...
        else if (byteCommand.Length == 12)
            command = BitConverter.ToInt32(byteCommand, 0);
        else if (byteCommand.Length == 16)
        ...
        if (targetDrone == 0) { 
            m_playerCommandPusherRSA.PushDroneCommandsToFocusDrone(joystickRightYPercent, joystickRightXPercent, joystickLeftYPercent, joystickLeftXPercent);
        }
        ...
        if (targetDrone >= 1 && targetDrone <= 12) {
            ...
            if (MSoccerMono_IsDroneExistingOnClient.IsOwnerOfFixedSoccerDrone(rsaKey, fixedSoccerId)) { 
                ...
                m_playerCommandPusherRSA.PushDroneCommandsToFixedSoccerIntIds(fixedSoccerId, joystickRightYPercent, joystickRightXPercent, joystickLeftYPercent, joystickLeftXPercent);
            }
    ...
    public void PushActionAsTextOnClient(string  command)
    {
        ...
        if(string.IsNullOrEmpty(targetDrone))
            return;
        if (MSoccerMono_IsDroneExistingOnClient.IsExistingAsAlias(targetDrone)) {
